...
<head>
    ...
    <style>
        ...
        /* Responsive Tweaks */
        @media (max-width: 1133px) { .scanner-box { max-width: 300px; max-height: 180px; } }
        @media (max-width: 768px) { .scanner-box { max-width: 300px; max-height: 180px; }}
...
<script>
    ...
    let scanCooldown = 3000;     // 2 seconds cooldown
   ...
   function refreshTable() {
    fetch("{{ route('attendances.list') }}")
        ...
        .then(data => {
            ...
            data.attendances.data.forEach((attendance, index) => {
            const row = document.createElement("tr");

            // Descending numbering
            const number = data.attendances.total - ((data.attendances.current_page - 1) * data.attendances.per_page) - index;

            row.innerHTML = `
                <td class="px-2 py-1">${number}</td>
                <td class="px-2 py-1">${attendance.first_name} ${attendance.last_name}</td>
                <td class="px-2 py-1">${attendance.division}</td>
                <td class="px-2 py-1">${attendance.time_in || ""}</td>
                <td class="px-2 py-1">${attendance.race_category}</td>
 ...
 function startScanner() {
    ...
    const DEVICE_TOKEN = @json(request()->cookie('device_token'));
