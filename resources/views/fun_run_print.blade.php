...
<head>
    ...
    <title>75th DSWD Anniversary</title>
    <style>
        /* Set the paper size (CR80) */
        @page {
            size: 85.60mm 70.00mm portrait; /* CR80 dimensions */
            margin: 2;
...
<body>
    <div>
        ...
        <h4>Fun Run Participants</h4>
        <p>{{$registration->division}}</p>

        <h5>QR Code</h5>

        <div class="qr-container">
            @if (!empty($registration->qr_number))
                {!! DNS2D::getBarcodeHTML($registration->qr_number, 'QRCODE', 5, 5) !!}
            @endif
        </div>
        <h3>{{ $registration->first_name }} @if($registration->middle_name)
            {{ Str::substr($registration->middle_name, 0, 1,) }}.
            @endif{{ $registration->last_name }}</h3>
    </div>
</body>
</html>
